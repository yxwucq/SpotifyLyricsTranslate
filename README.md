...
- **AI Translation / AI 翻译** — Claude / OpenAI, 8 target languages, streaming output (translations appear line-by-line), local cache
- **Song Meaning / 歌曲大意** — AI-generated background and theme interpretation, prompt follows target language
...
- Spotify `sp_dc` Cookie (optional — needed for Spotify official lyrics; without it, lyrics are fetched from LRCLIB)
...
1. Download `SpotifyLyrics.app.zip` from [Releases](https://github.com/yxwucq/SpotifyLyricsTranslate/releases)
2. Unzip and move to Applications
3. Remove macOS quarantine (unsigned app will show "damaged" otherwise):
   ```bash
   xattr -cr /Applications/SpotifyLyrics.app
   ```
4. Open SpotifyLyrics.app

### Build from Source / 从源码构建

```bash
git clone https://github.com/yxwucq/SpotifyLyricsTranslate.git
cd SpotifyLyricsTranslate
./build_app.sh
...
**sp_dc Cookie** (optional) — Provides access to Spotify's official synced lyrics for more songs. Without it, lyrics are fetched from LRCLIB. To get it: log in to [open.spotify.com](https://open.spotify.com) → DevTools → Application → Cookies → copy `sp_dc` value → paste in Settings → Credentials.
...
Translation toggle, song meaning, and other features are accessible from the menu bar. Appearance settings (font, color, scale, opacity, etc.) can be adjusted in Settings → Appearance. Click **About SpotifyLyrics** in the menu to check the current version.
